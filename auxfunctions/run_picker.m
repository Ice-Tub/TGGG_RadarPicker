...
%%
    ...
    ui_b = uicontrol('Parent',f,'Style','slider','Units','normalized','Position',bpos,...
                  'value',cini,'min',cmin,'max',cmax,'callback', @color_callback); % Color slider. Atm it uses fixed max and min values, instead they could be adopted to the file values.
    bgcolor = f.Color;
    ...
    if type_in == 1
        ...
        [layer,quality] = propagate_layer(layer,quality,geoinfo,tp,x_in,y_in,leftright,editing_mode);
        if isnewlayer
            [layer,quality] = propagate_layer(layer,quality,geoinfo,tp,x_in,y_in,-leftright,0);
        end
    elseif type_in==3
        if editing_mode
            del_min = max(1, x_in-tp.editing_window);
            del_max = min(length(layer), x_in+tp.editing_window);
        else
            del_min = 1;
